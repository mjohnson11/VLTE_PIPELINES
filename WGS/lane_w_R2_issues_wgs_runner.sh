...
#SBATCH -o ../../Output/shell_outs/lane_w_R2_issues_wgs_pipe_%A_%a.out      # File to which STDOUT will be written
#SBATCH -e ../../Output/shell_outs/lane_w_R2_issues_wgs_pipe_%A_%a.err      # File to which STDERR will be written
...
NGmerge -1 ${DATAB}${PRE}_${SAMP}${SUF}_R1_001.fastq.gz -2 ${DATAB}${PRE}_${SAMP}${SUF}_R2_001.fastq.gz -a -v -o ${OUTD}work/${SAMP}.trimmed
...
bwa mem -M -t 1 -R "@RG\tID:HJ25MBGXC\tSM:${SAMP}\tPL:ILLUMINA" ../../Output/WGS/reference/w303_vlte ${OUTD}work/${SAMP}.trimmed_1.fastq.gz ${OUTD}work/${SAMP}.trimmed_2.fastq.gz > ${OUTD}work/${SAMP}.sam 2> ${OUTD}logs/${SAMP}.bwa.log

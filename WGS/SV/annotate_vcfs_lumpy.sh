...
java -Xmx4g -jar ~/gatk/gatk-4.1.3.0/gatk-package-4.1.3.0-local.jar VariantsToTable -V ${OUTD}all_samples.smoove.square.ann.vcf -F CHROM -F POS -F REF -F ALT -F QUAL -F ANN -F SVTYPE -F SVLEN -F END -F ID -GF RO -GF AO -GF SU -GF AB -O ${OUTD}all_samples.smoove.square.ann.tsv

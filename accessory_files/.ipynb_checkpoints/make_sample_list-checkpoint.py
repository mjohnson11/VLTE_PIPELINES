...
d1['sample_vcf_combined_option'] = d1['samp'].apply(lambda s: '../../Output/WGS/work/' + s + '.g.vcf')
d1['sample_vcf'] = d1['samp'].apply(lambda s: '../../Output/WGS/work/' + s + '.g.vcf')
d2['sample_vcf_combined_option'] = d2['samp'].apply(lambda s: '../../Output/WGS/combined_work/' + s + '.g.vcf')
d2['sample_vcf'] = d2['samp'].apply(lambda s: '../../Output/WGS/work/' + s + '.g.vcf')
d1['sample_bam'] = d1['samp'].apply(lambda s: '../../Output/WGS/work/' + s + '.final.bam')
d2['sample_bam'] = d2['samp'].apply(lambda s: '../../Output/WGS/combined_work/' + s + '.final.bam')
d = pd.concat([d1[['Sample_ID', 'suffixes', 'samp', 'sample_vcf', 'sample_vcf_combined_option', 'sample_bam']], d2[['Sample_ID', 'suffixes', 'samp', 'sample_vcf', 'sample_vcf_combined_option', 'sample_bam']]])
...
d[['samp', 'sample_bam']].to_csv('bam_map_combined_option.txt', header=None, index=False, sep='\t')
